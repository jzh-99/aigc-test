...
- 每完成一轮长考、需求拆解、方案设计、技术选型，立刻把完整思考过程、推理逻辑、取舍- 理由 追加写入 .Codex/task-log.md
- 拆分出的子任务、执行步骤实时更新到 .Codex/task-plan.md，用待完成/进行中/已完成标记
- 每做完一个子步骤，先写日志、更新计划，再进入下一步，不累积超长上下文
- 任何时候如果会话中断、重启，优先读取 .Codex/task-log.md 和 .Codex/task-plan.md，从上次中断的思考节点继续，禁止从头重新长考
