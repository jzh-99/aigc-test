...
这些工作可以从运维和框架层面推进；涉及具体 AI Provider 调用、Agent 行为或业务状态机语义时，建议先与内部团队对齐。
...
2. 补充部署 checklist 和回滚 checklist。
3. 接入日志轮转。
4. 增强 `/healthz`。
5. 增加基础 CI：install、build、lint、test。
6. 增加队列积压和 Worker 失败告警。

这些工作能快速提升稳定性，也不会影响产品、AI 和 Agent 的内部设计。
