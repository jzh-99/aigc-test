...
| 字段 | 说明 | 示例 |
|------|------|------|
| `id` | 用户唯一 ID（UUID） | `a1b2c3d4-...` |
| `account` | 登录账号（邮箱或手机号） | `user@example.com` |
...
const ssoToken = jwt.sign(
  {
    sub: 'a1b2c3d4-e5f6-7890-abcd-ef1234567890', // 本平台 users.id
    email: 'user@example.com',                    // 本平台 users.account
...
String ssoToken = Jwts.builder()
    .claim("sub", "a1b2c3d4-e5f6-7890-abcd-ef1234567890")
...
```
https://<本平台域名>/login?token=<sso_token>&redirect=<目标路径>
```
...
| `redirect` | 否 | 登录后跳转的站内路径，默认 `/`。必须以 `/` 开头，不能跳转到外部域名 |

**示例**
```
https://toby.ai/login?token=eyJhbGci...&redirect=/canvas
```
...
```
https://u703085-af8b-f8f570d5.westx.seetacloud.com:8443/sso-test
```
...
|------|--------|
| JWT_SECRET | 联系管理员获取 |
| User ID | `954191f4-d11b-410e-8359-1b95436fdb5d` |
| Account | `hbtest@aigc.local` |
| 目标服务器域名 | `https://u703085-af8b-f8f570d5.westx.seetacloud.com:8443` |
| 跳转路径 | `/` |
