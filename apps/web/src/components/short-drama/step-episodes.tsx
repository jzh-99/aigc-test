...
export function StepEpisodes({ projectId, state, onStateChange }: StepEpisodesProps) {
  ...
  const pendingSegmentEpisodes = useMemo(
    () => episodes.filter(ep => ep.segments.length === 0).map(ep => ep.episodeNumber),
    [episodes]
  )
  ...
  const generateMissingSegments = async (episodeNumbers: number[]) => {
    ...
    try {
      ...
      toast.success(`已生成 ${episodeNumbers.length} 集分镜脚本`)
    } catch (err) {
  ...
  useEffect(() => {
    if (pendingSegmentEpisodes.length === 0 || isGeneratingSegments) return
  ...
  }, [projectId, pendingSegmentEpisodes, isGeneratingSegments])
