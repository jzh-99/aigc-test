...
describe('media-upload-rules', () => {
  ...
  test('空白画布上传允许图片和视频', () => {
    ...
    assert.equal(isFileAllowedForUploadTarget({ type: 'audio/wav' }, { kind: 'canvas' }), false)
  })

  test('图片和视频节点只允许上传同类资源', () => {
