  ...
  app.get<{ Params: { id: string } }>('/music/tracks/:id/events', async (request, reply) => {
    try {
      ...
      if (TERMINAL_STATUSES.has(snapshot.status)) {
        ...
        sendEvent('status', createMusicTrackSsePayload('status', snapshot))
        raw.end()
        return reply.hijack()
      }
      ...
      sub.on('message', async (_channel, message) => {
        try {
          ...
          if (!fresh) return
          if (event === 'lyrics_delta') {
            sendEvent(event, {
              event,
              ...(typeof parsed.data === 'object' && parsed.data !== null ? parsed.data : {}),
              track: fresh,
            })
          } else if (event === 'stream_url' || event === 'completed' || event === 'failed' || event === 'status') {
            sendEvent(event, createMusicTrackSsePayload(event, fresh))
          }
          if (event === 'completed' || event === 'failed') cleanup()
        } catch {
          const fresh = await getTrackSnapshot(db, request.params.id)
          if (!fresh) return
          const event: 'completed' | 'failed' | 'status' =
            fresh.status === 'completed' || fresh.status === 'failed' ? fresh.status : 'status'
          sendEvent(event, createMusicTrackSsePayload(event, fresh))
