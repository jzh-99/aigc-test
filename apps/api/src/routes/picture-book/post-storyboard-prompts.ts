...
const route: FastifyPluginAsync = async (app) => {
  app.post<{ Body: { project_id: string } }>(
    ...
    async (request, reply) => {
      ...
      generationLock = await acquireRedisLock(app.redis, `lock:picture-book:${request.body.project_id}:storyboard-prompts`)
      if (!generationLock) {
        return reply.status(409).send({
          error: { code: 'GENERATION_IN_PROGRESS', message: '绘本分镜正在生成中，请稍后刷新查看进度' },
        })
      }
