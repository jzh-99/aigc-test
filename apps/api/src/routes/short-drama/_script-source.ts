...
export function buildShortDramaScriptSummaryPrompts(state: ShortDramaState): {
  ...
  const systemPrompt = [
    ...
    isUploadedScript
      ? '只输出 JSON 对象，字段为 title、episodeCount 和 summary，不要输出 markdown、代码块或额外解释。episodeCount 必须是从原始剧本整体结构分析出的正整数。'
  ...
  const modeRequirements = isUploadedScript
    ? [
        ...
        '- 必须先分析原始剧本的整体结构、章节、场次、剧情节奏和信息密度，给出适合后续分集制作的 episodeCount。',
        '- episodeCount 必须是 1 到 100 之间的正整数；如果原剧本剧情容量超过 100 集，请压缩归并为不超过 100 集的连续短剧结构，不得返回大于 100 的值。',
        '- 如果原始剧本已有分集结构，请按原结构归纳 episodeCount；如果没有明确分集，请根据剧情容量推断合理集数。',
