...
import { runPurgeDeletedAssets } from '../jobs/purge-deleted-assets.js'
import { runBizMgmtOutboxPump } from '../jobs/biz-mgmt-outbox-pump.js'
...
export const CRON_JOB_PURGE_DELETED_ASSETS = 'purge-deleted-assets'
export const CRON_JOB_BIZ_MGMT_OUTBOX_PUMP = 'biz-mgmt-outbox-pump'
...
export async function scheduleCronJobs(): Promise<void> {
  ...
  await queue.upsertJobScheduler(
    CRON_JOB_PURGE_DELETED_ASSETS,
    { every: 24 * 60 * 60 * 1000 }, // 每天
    { name: CRON_JOB_PURGE_DELETED_ASSETS },
  )

  // 业管 outbox 派发泵：每 30 秒扫一次到期 pending 事件投到 biz-mgmt-notify-queue。
  // 这是 outbox 模式必需的投递环节（api 侧 enqueue 只写表不投队列）。
  // pump 内部有 Redis 分布式锁防多机重复扫描，dispatch 内部有 CAS 抢占防重复消费。
  await queue.upsertJobScheduler(
    CRON_JOB_BIZ_MGMT_OUTBOX_PUMP,
    { every: 30 * 1000 }, // 每 30 秒
    { name: CRON_JOB_BIZ_MGMT_OUTBOX_PUMP },
  )
  ...
  async (job) => {
    switch (job.name) {
      ...
      case CRON_JOB_PURGE_DELETED_PROJECTS:
        ...
        break
      case CRON_JOB_PURGE_DELETED_ASSETS:
        await runPurgeDeletedAssets()
        break
      case CRON_JOB_BIZ_MGMT_OUTBOX_PUMP:
        await runBizMgmtOutboxPump()
        break
      default:
