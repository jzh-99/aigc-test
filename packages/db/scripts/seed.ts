...
async function main() {
  ...
  // 2. Users / Teams / Workspaces — 已移除。
  // 业管先行登录改造（2026-06-26）：本地不再预置 seed 用户（admin/owner/editor），
  // 所有用户必须通过手机号登录、由业管平台 MEMBER-1001 判存亡后创建本地 user/team/workspace。
  // 如需清空历史 seed 残留的用户业务数据，执行 pnpm db:reset（见 scripts/reset-and-reseed.ts）。
  // 本地积分系统已退役（迁移 078），A 豆余额由业管平台统一管理。
